...
export default function Project({
  ...
  const [_, y] = useRecoilValue(scrollPositionState);
  const { width, height, orientation, size } = useViewport();
  const visibleMin = y > (height / 2) * (number + 1);
  const visibleMax = y < (height / 2) * (number + 2);
  ...
  return (
    <animated.li
      ...
      <div className={styles.inner} style={{ height, width: width }}>
